...
function W(t) {
	...
	for (let r = 0; r < n.length; r++) {
		const u = n[r].claim_order,
			a = (c > 0 && n[e[c]].claim_order <= u ? c + 1 : Q(1, c, (y) => n[e[y]].claim_order, u)) - 1;
		i[r] = e[a] + 1;
		const s = a + 1;
		(e[s] = r), (c = Math.max(s, c));
	}
	const f = [],
		l = [];
	let o = n.length - 1;
	...
	for (let r = 0, u = 0; r < l.length; r++) {
		...
		const a = u < f.length ? f[u] : null;
		t.insertBefore(l[r], a);
	}
}
function R(t, n) {
	if (w) {
...
function _t(t, n, e) {
	w && !e ? R(t, n) : (n.parentNode !== t || n.nextSibling != e) && t.insertBefore(n, e || null);
}
function U(t) {
...
function Ot(t, n, e, i, c, f, l, o = [-1]) {
	...
	if (
		((u.ctx = e
			? e(t, n.props || {}, (s, y, ...j) => {
					const k = j.length ? j[0] : y;
					return (
						u.ctx &&
							c(u.ctx[s], (u.ctx[s] = k)) &&
							(!u.skip_bound && u.bound[s] && u.bound[s](k), a && ct(t, s)),
						y
					);
			  })
			: []),
		...
		if (n.hydrate) {
			J();
			const s = Y(n.target);
			u.fragment && u.fragment.l(s), s.forEach(U);
